...
# Runs AbstractFFTs tests, relying on an FFT backend present in the environment.
# Ideally, the backend would be passed as an argument to make this pure, but this isn't possible
# since downstream implementations perform type piracy on AbstractFFTs.plan_fft in the current design.
function testfft()
